MODULE EquationOfStateModule_TABLE
  ...
  USE wlInterpolationModule, ONLY: &
    LogInterpolateSingleVariable_3D, &
...
CONTAINS
  ...
  SUBROUTINE ComputeTemperatureFromSpecificInternalEnergy_TABLE &
    ...
    CALL ComputeTempFromIntEnergy_Lookup &
           ( LOG10( D / ( Gram / Centimeter**3 ) ),   &
             LOG10( E / ( Erg / Gram ) + OS_E ),             &
             Y, LogDs_T, LogTs_T, Ys_T, &
             EOS % DV % Variables(iE_T) % Values, OS_E, &
             T_Lookup, Mask_Option = Mask_Option )
  ...
  SUBROUTINE ComputePressureFromSpecificInternalEnergy_TABLE &
               ( D, E, Y, P, Mask_Option )
  ...
  SUBROUTINE ComputeTemperatureFromPressure_TABLE( D, P, Y, T, Mask_Option )
    ...
    WRITE(*,*)
    WRITE(*,'(A4,A)') &
      '', 'WARNING: ComputeTemperatureFromPressure_TABLE not implemented'
    WRITE(*,*)
    STOP
    ...
    IF( ComputeDerivatives )THEN
      ...
      CALL ComputeDependentVariableAndDerivatives_TABLE &
             ( D(:), T(:), Y(:), E(:), TMP(:,1:3), iE_T, OS_E, &
               Units_V = Erg / Gram, Mask_Option = Mask_Option )
    ...
    IF( ComputeDerivatives )THEN
      ...
      CALL ComputeDependentVariableAndDerivatives_TABLE &
             ( D(:), T(:), Y(:), M(:), TMP(:,1:3), iMe_T, OS_Me, &
               Units_V = MeV, Mask_Option = Mask_Option )
  ...
  SUBROUTINE ComputeDependentVariable_TABLE &
               ( D, T, Y, V, iV, OS_V, Units_V, Mask_Option )
  ...
  SUBROUTINE ComputeDependentVariableAndDerivatives_TABLE &
               ( D, T, Y, V, dV, iV, OS_V, Units_V, Mask_Option )
    ...
    CALL LogInterpolateDifferentiateSingleVariable_3D  &
           ( LOG10( D(:) / ( Gram / Centimeter**3 ) ), &
             ...
             TMP, dTMP, Mask_Option = Mask_Option )
